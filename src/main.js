const { app, BrowserWindow, Menu, ipcMain, globalShortcut, session, screen, systemPreferences, shell } = require("electron");
const fs = require("fs");
...
function brandIconPath() {
  return path.join(__dirname, "assets/branding/app-logo.png");
}
...
function createControlWindow() {
  ...
  // in screenshots and screen captures. Only the teleprompter uses capture protection.
  ...
  const template = [
    {
      ...
      submenu: [
        ...
        { label: "重置悬浮位置", click: resetFloatingWindows },
        { type: "separator" },
    ...
    },
    {
      label: "窗口",
      submenu: [
        { label: "显示名片", accelerator: "Command+Shift+O", click: () => updateSettings({ overlay: { visible: true } }) },
        { label: "显示提词器", accelerator: "Command+Shift+P", click: () => updateSettings({ prompter: { visible: true } }) },
        { label: "重置悬浮位置", click: resetFloatingWindows }
      ]
    }
  ];
